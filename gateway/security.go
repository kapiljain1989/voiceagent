...
func NewPIIMasker() *PIIMasker {
	return &PIIMasker{
		...
		patterns: []piiPattern{
			...
			{
				Name:  "ssn_spoken",
				Regex: regexp.MustCompile(`(?i)\b(?:social security|social|ssn)\s+(?:number\s+)?(?:is\s+)?(?:(?:zero|one|two|three|four|five|six|seven|eight|nine)\s*){5,9}`),
				Mask:  "[SSN REDACTED]",
				Level: "critical",
			},
			{
				Name:  "credit_card_spoken",
				Regex: regexp.MustCompile(`(?i)(?:card|credit|debit|visa|mastercard|amex)\s+(?:number\s+)?(?:is\s+)?(?:(?:zero|one|two|three|four|five|six|seven|eight|nine)\s*){12,19}`),
				Mask:  "[CARD REDACTED]",
				Level: "critical",
			},
			{
				Name:  "cvv",
				Regex: regexp.MustCompile(`(?i)(?:cvv|cvc|security code|verification)\s+(?:is\s+|number\s+)?(\d{3,4})`),
				Mask:  "CVV [REDACTED]",
				Level: "critical",
			},
			{
				Name:  "dob",
				Regex: regexp.MustCompile(`(?i)(?:date of birth|birthday|born on|dob)\s+(?:is\s+)?(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})`),
				Mask:  "DOB [REDACTED]",
				Level: "high",
			},
			{
